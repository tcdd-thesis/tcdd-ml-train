...
def convert_to_segment(root: Path, sam_model: str, device) -> tuple[int, int]:
    ...
    - bbox lines  → converted using SAM via ultralytics yolo_bbox2segment
    ...
    for split in SPLITS:
        ...
        # yolo_bbox2segment converts bbox (5-field) lines using SAM
        # and keeps longer lines (segment/obb) as-is.
        # Output goes to  <split>/labels-segment/
        yolo_bbox2segment(
            im_dir=str(im_dir),
            save_dir=None,       # default: creates labels-segment next to im_dir
            sam_model=sam_model,
            device=device,
        )
